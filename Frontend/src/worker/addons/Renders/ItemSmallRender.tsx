...
function ItemSmallRender(props: Props) {
    return(
        <Link to={"/worker/items/" + props.item.id} className="w-full h-full max-h-71 bg-slate-50/20 rounded-xl backdrop-blur-lg shadow-xl p-4 flex flex-col gap-2 cursor-pointer transition-transform transform hover:-translate-y-1 cursor-pointer">
            <div>
                <img className="max-h-25 m-auto" src={props.item.image}></img>
            </div>
            <div className="w-full h-fit min-h-8 truncate max-h-10 p-1 text-center bg-slate-50/20 backdrop-blur-lg rounded-xl shadow-md font-semibold">
                {props.item.name}
            </div>
